...
import com.whatissatire.tutorialmod.block.custom.BismuthLampBlock;
...
public class ModBlockStateProvider extends BlockStateProvider {
    ...
    private void blockItem(DeferredBlock<?> deferredBlock) {
        simpleBlockItem(deferredBlock.get(), new ModelFile.UncheckedModelFile(MessageFormat.format("{0}:block/{1}", TutorialMod.MOD_ID, deferredBlock.getId().getPath())));
    }

    private void blockItem(DeferredBlock<?> deferredBlock, String appendix) {
        simpleBlockItem(deferredBlock.get(), new ModelFile.UncheckedModelFile(MessageFormat.format("{0}:block/{1}{2}", TutorialMod.MOD_ID, deferredBlock.getId().getPath(), appendix)));
    }
